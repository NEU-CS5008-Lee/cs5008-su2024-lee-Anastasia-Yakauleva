...
void addConnection(Graph* graph, char* person, char* friend){
    ...
    // If the person is not found, add the person to the graph
    if (p == -1) {
        if (n >= graph->numberOfVertices) {
            fprintf(stderr, "Error: Exceeding number of vertices\n");
            return;
        }
        p = n;
        graph->adjLists[p] = createNode(person);
        n++; // increment n as we have added a new person
    }

    // Search for the friend in the graph
    int fr = search(friend, graph);
    if (fr == -1) {
        if (n >= graph->numberOfVertices) {
            fprintf(stderr, "Error: Exceeding number of vertices\n");
            return;
        ...
        fr = n;
        graph->adjLists[fr] = createNode(friend);
    }

    // Insert the person into the adjacency list of the friend
    node* newPersonNode = createNode(person);
    newPersonNode->next = graph->adjLists[fr]->next;
    graph->adjLists[fr]->next = newPersonNode;

    // Insert the friend into the adjacency list of the person
